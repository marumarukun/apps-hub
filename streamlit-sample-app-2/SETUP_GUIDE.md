...
**新しいセットアップガイドはプロジェクトルートの `SETUP_GUIDE.md` をご利用ください。**

Apps Hubの新しい二階層アーキテクチャでは、個別アプリのセットアップ手順が大幅に簡略化されました。

## 🚀 新しい手順（簡単！）

1. **プロジェクト全体のセットアップ**: ルートの `SETUP_GUIDE.md` に従って初期設定を実行
2. **共有インフラのデプロイ**: `infrastructure.yml` ワークフローを実行
3. **このアプリのデプロイ**: `streamlit-sample-app.yml` ワークフローを実行

**それだけ！IP制限もロードバランサも全て自動設定されます。**
...
**⚠️ 注意**: 以下の手順は新アーキテクチャでは不要です。参考のためのみ残してあります。
